...
def compare_evidence(claim_obj, sources):
    ...
    _evidence_snippets = [{'source': s.get('source'), 'snippet': s.get('snippet')} for s in sources]
    prompt = f"""
Compare the following Factual Claim against the retrieved Search Evidence snippets.
Only consider TRUSTED sources: official government websites, official organizations,
primary documents, official announcements, reputable established news organizations
(Reuters, AP, BBC, AFP, The Hindu, Indian Express, Hindustan Times, Times of India,
NDTV, India Today, established fact-checking organizations).
Do NOT count random blogs, unknown websites, SEO sites, Reddit, Quora, social media,
Facebook, Instagram, X/Twitter, YouTube, or AI-generated websites as trusted.

Claim: "{claim_text}"

Evidence Snippets:
{json.dumps(_evidence_snippets, indent=2)}

A claim is VERIFIED only when sufficient evidence from a TRUSTED and RELEVANT source
actually supports the claim (not just keyword overlap).
A claim is UNVERIFIED when trusted evidence cannot be found — even if there is
contradicting evidence (do NOT use a FALSE/CONTRADICTED status).

Return JSON object in format:
{{
  "status": "VERIFIED" | "UNVERIFIED",
  "supporting_sources": ["Trusted Source Name 1"],
  "contradicting_sources": [],
...
"""
    ...
    if isinstance(llm_eval, dict) and 'status' in llm_eval:
        raw_status = llm_eval.get('status', 'UNVERIFIED').upper().strip()
        # Collapse any old statuses into the two allowed statuses
        if raw_status in ('VERIFIED', 'SUPPORTED', 'HIGHLY_SUPPORTED', 'CONFIRMED', 'TRUE', 'VERIFIED_TRUE'):
            normalized = 'VERIFIED'
        else:
...
def _local_compare_evidence(claim_text, sources):
    ...
    Only TRUSTED sources (Tier 1 or Tier 2) can make a claim VERIFIED.
    All other outcomes → UNVERIFIED.
    """
    import source_analysis as _sa
    ...
    for src in sources:
        ...
        if overlap >= 2:
            # Only count as evidence if from a trusted tier
            tier_info = _sa.classify_source_tier(url, src_name)
            if tier_info['tier'] in ('TIER_1_OFFICIAL', 'TIER_2_ESTABLISHED_NEWS'):
                trusted_supporting.append(src_name)

    if trusted_supporting:
        status = 'VERIFIED'
        summary = f"Claim verified by {len(trusted_supporting)} trusted source(s) including {trusted_supporting[0]}."
    else:
        status = 'UNVERIFIED'
        summary = "No sufficient trusted evidence found to independently verify this claim."

    return {
        'claim': claim_text,
        'status': status,
        'supporting_sources': list(set(trusted_supporting)),
        'contradicting_sources': [],
        'summary': summary
    }
...
def normalize_claim_status(status_str):
    """
    Normalizes claim verification status into exactly two categories:
    VERIFIED or UNVERIFIED.

    VERIFIED  — sufficient trusted evidence supports the claim.
    UNVERIFIED — trusted evidence could not be found (does NOT mean proven false).
    """
    if not status_str or not isinstance(status_str, str):
        return 'UNVERIFIED'
    st = status_str.upper().strip()
    if st in ('VERIFIED', 'SUPPORTED', 'TRUE', 'VERIFIED_TRUE', 'CONFIRMED', 'HIGHLY_SUPPORTED'):
        return 'VERIFIED'
    else:
        # Everything else (UNVERIFIED, CONTRADICTED, FALSE, PARTIALLY_SUPPORTED, etc.)
        # collapses to UNVERIFIED at the claim level.
        return 'UNVERIFIED'


def determine_ai_prediction(claims=None):
    """
    Two-status overall article rule:
      - IF at least ONE major claim is VERIFIED  → REAL (article is TRUE)
      - IF ALL major claims are UNVERIFIED        → FAKE (article is FALSE)

    UNVERIFIED claims alone do NOT make the article FAKE
    as long as at least one claim is VERIFIED.
    """
    ...
    # ANY VERIFIED claim → REAL (TRUE)
    if 'VERIFIED' in normalized_statuses:
        return 'REAL'

    # ALL UNVERIFIED → FAKE (FALSE)
    return 'FAKE'
...
def verify_article(article_text, article_url=""):
    ...
    Claim-Level Rule (two statuses only):
      - Extracts all major claims
      - Verifies every claim against web & reputable sources
      - Claim is VERIFIED when trusted evidence supports it
      - Claim is UNVERIFIED when trusted evidence cannot be found
      - ANY claim VERIFIED  → REAL (article is TRUE)
      - ALL claims UNVERIFIED → FAKE (article is FALSE)
    """
    if not article_text or not isinstance(article_text, str) or not article_text.strip():
        return {'error': 'Please provide a valid news article text to verify.'}
    ...
    # Calculate claim-level breakdown (two statuses: VERIFIED / UNVERIFIED)
    verified_cnt = 0
    unverified_cnt = 0
    ...
    # 6. Apply Two-Status Claim Rule:
    # ANY VERIFIED → REAL | ALL UNVERIFIED → FAKE
    ai_prediction = determine_ai_prediction(claim_verifications)

    if ai_prediction == "REAL":
        overall_status = "SUPPORTED"
        verified_count_label = verified_cnt
        total_claims_label = len(extracted_claims)
        if unverified_cnt > 0:
            ai_assessment = (
                f"{verified_cnt} of {total_claims_label} major claims were verified using trusted sources. "
                f"The remaining {unverified_cnt} claim(s) could not be independently verified."
            )
        else:
            ai_assessment = (
                f"All {total_claims_label} major claims were verified using trusted external sources."
            )
    else:
        overall_status = "UNVERIFIED"
        ai_assessment = (
            "No major claims could be independently verified using the trusted sources checked."
        )
    ...
    print("Verified Claims:", verified_cnt)
    ...
    report = {
        ...
        'verification_summary': {
            ...
            'verified_claims': verified_cnt,
            'unverified_claims': unverified_cnt,
            # Legacy aliases kept for backward compatibility
            'true_claims': verified_cnt,
            'false_claims': 0,
        ...
        'overall_status': overall_status,
        'limitations': [
            "A claim is VERIFIED when sufficient trusted evidence is found from official or established news sources.",
            "A claim is UNVERIFIED when trusted evidence cannot be found — this does not mean the claim is proven false.",
            "The article is TRUE if at least one major claim is VERIFIED.",
            "The article is FALSE only if ALL major claims are UNVERIFIED."
        ]
    }
