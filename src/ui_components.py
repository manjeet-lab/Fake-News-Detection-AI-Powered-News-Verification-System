...
def render_status_badge(status):
    ...
    Claim-level statuses: VERIFIED (green) or UNVERIFIED (grey).
    """
    ...
    # Normalize to two allowed claim statuses for display
    if st_clean in ('VERIFIED', 'SUPPORTED', 'REAL', 'HIGHLY_SUPPORTED', 'CONFIRMED', 'TRUE', 'ONLINE', 'ACTIVE',
                    'FACT_CHECKED_TRUE', 'FACT_CHECKED_MOSTLY_TRUE'):
        display_label = st_clean  # keep original label for context (e.g. FACT_CHECKED_TRUE)
        bg_color = "rgba(16, 185, 129, 0.15)"
        border_color = "rgba(16, 185, 129, 0.4)"
        text_color = "#34d399"
        icon_name = "check-circle"
    elif st_clean in ('READY',):
        display_label = st_clean
        bg_color = "rgba(245, 158, 11, 0.15)"
        border_color = "rgba(245, 158, 11, 0.4)"
        text_color = "#fbbf24"
        icon_name = "alert-triangle"
    elif st_clean in ('FAILED',):
        display_label = st_clean
        ...
        # UNVERIFIED, CONTRADICTED, FALSE, PARTIALLY_SUPPORTED, UNKNOWN, UNAVAILABLE → all shown as UNVERIFIED
...
def render_hero_assessment_card(final_dec):
    """
    Renders the primary Final Assessment verdict card.
    Only two possible outcomes: TRUE (green) or FALSE (red).
    """
    f_label = final_dec.get('final_label') or final_dec.get('final_decision') or 'FALSE'
    f_level = final_dec.get('evidence_level') or final_dec.get('sub_status') or 'CLAIMS UNVERIFIED'
    reason = final_dec.get('reason', 'Evidence evaluation completed.')

    # Normalise: REAL/TRUE both map to green TRUE; everything else to red FALSE
    if f_label in ('TRUE', 'REAL'):
        display_label = 'TRUE'
        border_col = "#10b981"
        grad_bg = "linear-gradient(135deg, rgba(6, 78, 59, 0.6) 0%, rgba(2, 44, 34, 0.8) 100%)"
        text_col = "#34d399"
        icon_name = "check-circle"
    else:  # FALSE / FAKE / any other
        display_label = 'FALSE'
        border_col = "#ef4444"
    ...
    ">
        ...
        <div style="display: flex; align-items: center; justify-content: center; gap: 12px; margin: 6px 0;">
            {hero_svg}
            <span style="font-size: 2.75rem; font-weight: 800; color: {text_col}; letter-spacing: 0.04em;">{display_label}</span>
        </div>
