...
if nav_choice == "Analyze News":
    ...
    if res:
        if 'error' in res or res.get('extraction_status') == 'FAILED':
            ...
            with m_col1:
                render_metric_card("Verified Claims", es.get('verified_claims', es.get('claims_supported', 0)), icon_name="check-circle")
            with m_col2:
                render_metric_card("Unverified Claims", es.get('unverified_claims', 0), icon_name="info")
            with m_col3:
            ...
            with r_tab1:
                ...
                if not claims_list:
                    ...
                    for i, c in enumerate(claims_list, 1):
                        ...
                        # Normalize for display: VERIFIED → VERIFIED, everything else → UNVERIFIED
                        from ai_verification import normalize_claim_status
                        display_status = normalize_claim_status(c_status)

                        with st.expander(f"Claim {i:02d}: \"{c_text[:75]}...\"", expanded=(i == 1)):
                            st.markdown(f"**Full Claim:** {c_text}")
                            st.write("**Verification Status:**")
                            render_status_badge(display_status)
                            st.markdown(f"**Evidence Summary:** {c_sum}")

                            if isinstance(c, dict) and c.get('supporting_sources'):
                                st.markdown("**Trusted Supporting Sources:**")
                                for s in c['supporting_sources']:
                                    render_external_link("Read Trusted Source", s)
